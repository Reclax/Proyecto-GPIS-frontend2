...
function GestionProductosPage() {
        ...
        const mapped = (Array.isArray(data) ? data : []).map((p) => {
          const firstPhoto =
            p.ProductPhotos && p.ProductPhotos.length > 0
              ? p.ProductPhotos[0].url
              : null;
          const foto = firstPhoto
  ...
  const productosFiltrados = productos.filter((prod) => {
    ...
    const matchEstado =
      filterEstado === "todos" || prod.estado === filterEstado;
    const matchTipo = filterTipo === "todos" || prod.tipo === filterTipo;
    const matchModeracion =
      filterModeracion === "todos" ||
      prod.moderationStatus === filterModeracion;
    // Origen: marca automática si fue bloqueado por incidencia/reporte (simulado si _raw tiene incidenceCount/reportCount)
    const origen =
      prod._raw?.lastModerationSource ||
      (prod._raw?.incidenceCount > 0
        ? "incidencia"
        : prod._raw?.reportCount > 0
        ? "reporte"
        : "ninguno");
    const matchOrigen = filterOrigen === "todos" || origen === filterOrigen;

    return (
      matchSearch && matchEstado && matchTipo && matchModeracion && matchOrigen
    );
  });

  // Marcar como peligroso
  const marcarComoPeligroso = (producto) => {
    console.log("marcarComoPeligroso llamado con:", producto);
    setModalData({
      isOpen: true,
      type: "warning",
      title: "Reportar Producto",
      message: `¿Estás seguro de reportar "${producto.titulo}"?\n\nEsto creará un reporte administrativo que aparecerá en "Gestión de Incidencias" para ser asignado a un moderador.`,
      onConfirm: async () => {
        try {
          ...
          if (!currentUser || !currentUser.id) {
            throw new Error(
              "No se pudo obtener el usuario actual. Por favor, inicia sesión nuevamente."
            );
          }

          // 2. Crear el reporte administrativo en la base de datos
          console.log("Creando reporte para producto ID:", producto.id);
          ...
          setModalData({
            ...
            message:
              'El reporte ha sido creado exitosamente. Ahora puedes ir a "Gestión de Incidencias" para asignar este reporte a un moderador.',
      ...
      onConfirm: async () => {
        try {
          ...
          setModalData({
            ...
            message:
              "El producto ha sido reactivado y está disponible nuevamente en la plataforma.",
            confirmText: "Entendido",
          });
        } catch {
          setModalData({
      ...
      <div className="sb-container max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">
            Administra y supervisa todos los productos y servicios publicados en
            la plataforma
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-7 gap-3 pt-6 border-t border-gray-200">
            ...
            <div className="bg-orange-50 rounded-lg p-3 text-center border border-orange-200">
              <p className="text-lg font-bold text-orange-600">
                {
                  productos.filter((p) => p.moderationStatus === "review")
                    .length
                }
              </p>
                      ...
                      if (catId) {
                        ...
                        if (cached) {
                          ...
                          if (
                            ...
                            producto.categoria.startsWith("ID ")
                          ) {
                            setProductos((prev) =>
                              prev.map((p) =>
                                p.id === producto.id
                                  ? { ...p, categoria: cached.name }
                                  : p
                              )
                            );
                          }
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <FiChevronDown
                      className={`text-2xl text-gray-400 transition-transform flex-shrink-0 ${
                        expandedId === producto.id ? "rotate-180" : ""
                      }`}
                    />
                  </div>
                </div>
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          {producto.fecha_suspension && (
                            <div>
                              ...
                              <p className="text-gray-900">
                                {formatDate(producto.fecha_suspension)}
                              </p>
                            </div>
                          )}
                          {producto.reportes > 0 && (
                            <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                              <p className="text-purple-900 font-semibold">
                                {producto.reportes} reportes de usuarios
                              </p>
                            </div>
                          )}
                        </div>
