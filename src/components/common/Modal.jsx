...
function Modal({
  ...
  const isConfirmType =
    type === "confirm" || type === "login" || type === "warning";
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        {/* Header */}
        <div
          className={`${colors.bg} px-6 py-4 rounded-t-2xl border-b ${colors.border} flex items-center justify-between`}
        >
          ...
          <button
            onClick={async () => {
              if (isConfirmType && onConfirm) {
                await onConfirm();
                // No cerramos el modal aquí - el onConfirm lo manejará
              } else {
                onClose();
              }
            }}
