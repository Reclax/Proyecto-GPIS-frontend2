...
// URL base del backend - Lee desde variable de entorno o usa IP del servidor como fallback
const API_BASE_URL =
  import.meta.env.VITE_API_URL || "http://20.48.177.225:3020";
export { API_BASE_URL };
...
export const productAPI = {
  ...
  updateProduct: async (productId, productData, photos = null) => {
    ...
    formData.append(
      "locationCoords",
      JSON.stringify(productData.locationCoords || { lat: null, lng: null })
    );
  ...
  updateModerationStatus: async (productId, moderationStatus) => {
    const response = await api.put(`/products/${productId}/moderation`, {
      moderationStatus,
    });
    return response.data;
  },
